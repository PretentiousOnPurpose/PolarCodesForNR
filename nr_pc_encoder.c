...
int * NR_PC_GET_REL_SEQ(struct PC_CONFIG * pcConfig) {
    ...
    int * rel_seq = (int *)calloc(pcConfig->N, sizeof(int));
    
    while (iter_seq < pcConfig->N) {
        if (MASTER_REL_SEQ[iter_seq] < pcConfig->N) {
            *(rel_seq + iter_seq) = MASTER_REL_SEQ[iter_seq];
            iter_seq++;
        }
    }
...
void NR_PC_GET_N(struct PC_CONFIG * pcConfig) {
    ...
    if ((double)pcConfig->E <= (9/8) * pow(2, ceil(log2(pcConfig->E)) - 1) || ((double)pcConfig->K/pcConfig->E < (9/16)) ) {
        n1 = ceil(log2(pcConfig->E)) - 1;
    } else {
        n1 = ceil(log2(pcConfig->E));
    }

    n2 = ceil(log2(pcConfig->K/Rmin));
...
int * PC_SET_DATABITS(int * dataBits, struct PC_CONFIG * pcConfig) {
    ...
    for (iter_bits = 0; iter_bits < pcConfig->K; iter_bits++) {
        *(encData + rel_seq[pcConfig->N - iter_bits]) = *(dataBits + iter_bits);
    }  
